...
/**
 * Non-RPC CORE for the owner-gated admin actions (Phase 13 operator panel +
 * Phase 17 pod-config editor).
 *
 * SECURITY (CR-01 / CR-02): this module does NOT carry the `"use server"`
 * directive, so nothing here is a network-reachable Server Action — these are
 * plain server-only functions. The `import "server-only"` marker makes any
 * accidental client-bundle import a BUILD error. The RPC entry points live in
 * `@/lib/admin-actions` (`"use server"`); each public action there derives the
 * caller identity from the live session ONLY and delegates to the matching
 * `*Core` impl below. The `actor` / `auth` / `db` / `deps` seams on the `*Core`
 * functions exist EXCLUSIVELY for in-process unit tests — they are never part
 * of any serialized argument surface reachable from the browser.
 *
 ...
 *
 * NOT a Server Action (CR-02): it lives here in the non-`"use server"` core so
 * it can never be RPC-invoked to forge arbitrary audit rows. Only the in-process
 * `*Core` impls call it, and only with a server-derived actor.
 *
 ...
 */
export async function inviteOperatorCore(args: {
  actor?: Actor;
  name?: string;
  email: string;
  auth?: AuthLike;
 ...
 */
export async function removeOperatorCore(args: {
  actor?: Actor;
  auth?: AuthLike;
  targetId: string;
  targetEmail?: string;
 ...
 */
export async function resetOperatorPasswordCore(args: {
  actor?: Actor;
  auth?: AuthLike;
  email: string;
  targetId?: string;
 ...
 */
export async function resetOperator2FACore(args: {
  actor?: Actor;
  auth?: AuthLike;
  db?: unknown;
  targetId: string;
 ...
 */
export async function changePasswordCore(args: {
  actor?: { id?: string; email?: string };
...
export interface PodConfigDeps {
 ...
 */
export async function updatePodConfigCore(args: {
  actor?: Actor;
  auth?: AuthLike;
  db?: unknown;
  field: string;
 ...
 */
export async function updatePodConfigBoundCore(args: {
  actor?: Actor;
  auth?: AuthLike;
  db?: unknown;
  field: string;
