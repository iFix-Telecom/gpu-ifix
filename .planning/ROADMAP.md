...
- [ ] 14-01-PLAN.md — Wave 1: gateway device-gate (Wave 0 test rework RED→GREEN) — WhisperDevice on primaryPodURLs, gate 3 stt override sites on device==cuda, delete Config.PrimaryPodServeSTT, wire main.go DeviceReport fetch
- [ ] 14-02-PLAN.md — Wave 2: pod VRAM-adaptive whisper — onstart.go nvidia-smi device export + :9100 /whisper_device responder, drop supervisord.conf:46 cpu pin (keep HF_HUB_CACHE), forward -p 9100:9100
- [ ] 14-03-PLAN.md — Wave 3 (autonomous:false): rebuild+push+promote image + uat-14.sh + 2 live Vast UATs (1×3090→gemini; ≥30GB→pod-GPU <5s no OOM) + remove prod PRIMARY_POD_SERVE_STT + 14-VERIFICATION rollup
