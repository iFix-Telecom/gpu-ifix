...
### Phase 16: Close gap TEN-04 — wire STT/embed usage metering (CLOSURE, from v1 re-audit 2026-06-28)
...
- [x] 17-02-PLAN.md — `podconfig` package: Loader (atomic snapshot + last-good) + ListenAndReload (pod_config_changed) + reload metric + tests

**Wave 3** *(depends 17-02)*

- [x] 17-03-PLAN.md — Reconciler/budget/schedule leem os 16 hot fields do snapshot (estrutural fica em cfg) + main.go wiring + seed env→DB idempotente

**Wave 4** *(depends 17-03)*

- [x] 17-04-PLAN.md — Gateway admin endpoints: GET /admin/primary/lifecycle (live status) + PATCH /admin/primary/config (write+bounds-validate) + mounts

**Wave 5** *(depends 17-04)*

- [x] 17-05-PLAN.md — Dashboard server actions updatePodConfig/Bound (requireOwner + validação + audit) + server-only admin-key helper + fetchPrimaryLifecycle

**Wave 6** *(depends 17-05)*
...
- [x] 18-01-PLAN.md — Gateway: handlers admin de tenant (list/create 409-dup) + key (list/create raw-1×-sem-hash/revoke-idempotente) + mounts no adminRouter + testes fake-queries (TEN-UI-01..05)
...
- [x] 18-03-PLAN.md — Dashboard server actions owner-gated createTenant/createTenantKey/revokeKey (requireOwner 1º + validação + 1 audit/ação sem raw) + wrappers + testes (TEN-UI-08/09)
...
- [x] 19-02-PLAN.md — Wave 2: deploy consolidated gateway + embed swarm stacks (Portainer compose-string, endpoint 6, DIGEST-pinned) → bd_ai_gateway + R2 + fresh infra-redis-1 + local embed (verified from gateway netns); 172.18.0.1:18000 documented tier-1-only; validate green with DEV tenant (parallel to live prod) — stacks 38 (gateway+redis) + 39 (embed); chat/embed/admin 200 + billing 918→923; prod unaffected
...
- [x] 22-04-PLAN.md — CV-02: gateway serve chat Gemini (decisão A upstream-nativo vs B alias→openrouter-chat, provada por path-test) + validar tool_choice
...
**Requirements**: FF-01 (created_budget_s fast-fail, regime 1), FF-02 (progress_stall_budget_s fast-fail, regime 3, via onstart-log heartbeat), FF-03 (fonte de progresso: Vast logs API primário `PUT /instances/request_logs/{id}/`→result_url→GET NESTA phase; SSH-tail `/var/log/onstart.log` fallback DEFERIDO p/ follow-up — split formal pós-review codex, seguro pq telemetria-vazia=unknown-não-mata (rides ao coldstart ceiling); status_msg+actual_status do GetInstance cobre regimes 1/2 de graça), CFG-01 (migration pod_config +created_budget_s +progress_stall_budget_s +min/max, molde dos budgets existentes; config_write.go 2 cases PATCH), UI-01 (dashboard 2 sliders, molde Phase 17), BL-01 (auto-blocklist quando fail_streak≥2 — REUSA CountConsecutiveFailedPrimaryProvisions existente, NÃO novo contador; +remove da allowlist; persiste pod_config.vast_machine_blocklist; sem TTL agora, prune manual — ponytail:), AL-01 (auto-allowlist no first_health_pass +remove do blocklist; cap ~20 FIFO+dedup; query UpdatePodConfigFieldAllowlist espelha a de blocklist), OBS-11 (download-weights.sh dropar `mc cp --quiet` → heartbeat mid-file, habilita FF-02) — LOCKED em 20-CONTEXT.md
