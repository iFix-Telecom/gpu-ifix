...
| 18-02-01 | 02 | 2 | TEN-UI-07, TEN-UI-10 (env) | T-18-03, T-18-10 | gateway-admin POST+PATCH server-only (POST retorna corpo JSON parseado do create-key, nunca key_hash/key_lookup_hash); leak-guard {route.ts,gateway-admin.ts}; stack 40 → gateway consolidado | source+behavior | `bunx vitest run src/lib/gateway.test.ts -t leak` | ✅ existente | ⬜ pending |
